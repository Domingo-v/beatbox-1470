...
def preprocess_dataset(input_dir, metadata_csv, output_dir):
    """
    ...
    #NOTE PREVIOUS WAV IMPLEMENTATION
    ...
    for root, _, files in os.walk(input_dir):
        ...
        for fn in audio_files:
            track_id, _ = os.path.splitext(fn) #identifier is within the filename
            try:
                genre = genre_map[int(track_id)]
            except (KeyError, ValueError):
                print(f"[SKIP] unknown track_id {track_id}")
                continue
            
            #output folder structuring
            in_path = os.path.join(root, fn)
            out_folder = os.path.join(output_dir, genre)
            os.makedirs(out_folder, exist_ok=True)
            out_path = os.path.join(out_folder, f"{track_id}.npy")

            try:
                song = preprocess_clip(in_path) #individual clip processing
                np.save(out_path, song)
                print(f"[OK] {in_path} → {out_path}  {song.shape}")
            except Exception as e:
                print(f"[ERR] {in_path}: {e!r}")


def main():
    #NOTE OLD MAIN RELATING TO INITIAL SMALLER DATASET
#     if len(sys.argv) != 3:
...
#     print("PREPROCESSED ALL FILES")
    if len(sys.argv) != 4:
        print("Usage: python preprocess.py /path/to/audio_root /path/to/tracks.csv /path/to/output_dir")
        sys.exit(1)
    audio_root, metadata_csv, output_dir = sys.argv[1:]
    preprocess_dataset(audio_root, metadata_csv, output_dir)
